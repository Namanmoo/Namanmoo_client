...
아래에 **색 채우기 + 확장 팔레트 12색**을 더 얹었다(AIGame 그리기 도구와 같은 구성).
지금 고른 도구와 색은 아이콘 아래 **주황 밑줄**로 표시된다 — 목업 그림을 덮지 않기 위해
밑줄을 골랐다.

**색 채우기**는 목업 도구바에 빈 자리가 없어 팔레트 줄 맨 앞 칸에 회색 "채움" 버튼으로
넣었다. 스캔라인 플러드 필이고([DrawingFill.cs](../Assets/Scripts/Forge/DrawingFill.cs)),
허용 오차 32를 둔다 — 손그림 외곽선은 안티에일리어싱돼 있어 0이면 선 안쪽에 얇은 띠가
남는다. 선이 끊겨 있으면 새어 나가는데, 그게 버킷의 정상 동작이라 그대로 두고 테스트로
기대를 고정했다(되돌리기로 복구된다).
